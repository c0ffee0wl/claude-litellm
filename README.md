...
- **Model naming**: Claude Code asks for the upstream id directly (`azure/gpt-5.6-terra` for Sonnet/Opus, `azure/gpt-5.6-luna` for Haiku, `azure/gpt-5.6-sol` for Fable — the premium tier, which needs its own Azure deployment) via `ANTHROPIC_DEFAULT_{HAIKU,SONNET,OPUS,FABLE}_MODEL` in `~/.profile`. LiteLLM's `model_list` supplies the Azure endpoint and key but adds no alias layer, and it targets Azure's v1 surface (`…/openai/v1`) so every request goes out through the Responses API. Anthropic's `/v1/messages` format stays intact the whole way.
