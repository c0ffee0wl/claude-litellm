...
|---|---|
| `./linux/setup.sh` | Full setup: LiteLLM + Claude Code + managed-settings hardening + ACP + claude-history |
| `./linux/setup.sh --router-only` | LiteLLM + Claude Code + claude-history. Skips ACP and managed-settings hardening. Dev-box mode |
...
- **Conversation history**: claude-run UI at `http://127.0.0.1:12001`.
...
- `linux/setup.sh`: phases 1-9 (see `CLAUDE.md`)
