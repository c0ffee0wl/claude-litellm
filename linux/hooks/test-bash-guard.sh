...
check 2 "$F" 'for d in a b; do rm -rf "$d"; done'  'rm: for-loop body (BYPASS)'
check 2 "$F" 'while true; do rm -rf x; done'       'rm: while-loop body (BYPASS)'
check 2 "$F" 'if true; then rm -rf x; fi'          'rm: then branch (BYPASS)'
check 2 "$F" 'if false; then :; else rm -rf x; fi' 'rm: else branch (BYPASS)'
check 2 "$F" '{ rm -rf x; }'                       'rm: brace group (BYPASS)'
check 2 "$F" 'VAR=1 rm -rf x'                      'rm: env assignment prefix (BYPASS)'
